...
//warning timer for match time
  if (frc::Timer::GetMatchTime() < 20 && frc::Timer::GetMatchTime() > 16){
    warning_timer.Start();
    if (warning_timer.Get() < 1 || warning_timer.Get() > 1.5 ){
      frc::SmartDashboard::PutBoolean("Time Remaining", true);
      Robot::m_oi.SetOperatorRumbleLeft(0.5);
      Robot::m_oi.SetOperatorRumbleRight(0.5);
    } else if(warning_timer.Get() > 3){
      frc::SmartDashboard::PutBoolean("Time Remaining", false);
      Robot::m_oi.SetOperatorRumbleLeft(0);
      Robot::m_oi.SetOperatorRumbleRight(0);
      warning_timer.Reset();
    }
  }else if(frc::Timer::GetMatchTime() < 5 && frc::Timer::GetMatchTime() > 2){
    warning_timer.Start();
     if (warning_timer.Get() == 1 || warning_timer.Get() == 1.5 || warning_timer.Get() == 2){
       frc::SmartDashboard::PutBoolean("Time Remaining", true);
       Robot::m_oi.SetOperatorRumbleLeft(0.5);
       Robot::m_oi.SetOperatorRumbleRight(0.5);
     } else if(warning_timer.Get() > 2){
       frc::SmartDashboard::PutBoolean("Time Remaining", false);
       Robot::m_oi.SetOperatorRumbleLeft(0);
       Robot::m_oi.SetOperatorRumbleRight(0);
       warning_timer.Reset();
