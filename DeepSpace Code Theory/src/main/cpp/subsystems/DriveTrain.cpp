...
void DriveTrain::SetSpeed(double left, double right) {
  ...
  .Add("Left Drivetrain", left_velocity);

  frc::Shuffleboard::GetTab("Debugging")
  .Add("Right Drivetrain", right_velocity);
}
...
double DriveTrain::GetLeftDistance() {
  frc::SmartDashboard::PutNumber("Left Ticks", m_left_back.GetSensorCollection().GetQuadraturePosition() - initial_left_position);
	return m_left_back.GetSensorCollection().GetQuadraturePosition() - initial_left_position;
}

double DriveTrain::GetRightDistance() {
  frc::SmartDashboard::PutNumber("Right Ticks", m_right_middle.GetSensorCollection().GetQuadraturePosition() - initial_right_position);
  return m_right_middle.GetSensorCollection().GetQuadraturePosition() - initial_right_position;
}

double DriveTrain::GetDistance() {
  frc::SmartDashboard::PutNumber("Average Distance", ((GetRightDistance() - GetLeftDistance()) / 2) * (6 * pi / 4096));
  return ((GetRightDistance() - GetLeftDistance()) / 2) * (6 * pi / /*4096*/1024);
}
...
double DriveTrain::GetAngle() {
  frc::SmartDashboard::PutNumber("Angle", NavX->GetAngle() + initial_angle);
  return NavX->GetAngle() + initial_angle;
}

double DriveTrain::GetTargetAngle() {
  if (z_position <= hab_line) {
    frc::SmartDashboard::PutString("Zone", "Human Player Zone");
    return GetAngle() - human_player_station_angle;
  } else if (z_position <= upper_zone_line) {
    if (x_position <= left_mid_line) {
      frc::SmartDashboard::PutString("Zone", "Front Left Rocket");
      return GetAngle() - front_left_rocket_angle;
    } else if (x_position < right_mid_line) {
      frc::SmartDashboard::PutString("Zone", "Front Cargo Ship");
      return GetAngle() - front_cargo_ship_angle;
    } else {
      frc::SmartDashboard::PutString("Zone", "Front Right Rocket");
      return GetAngle() - front_right_rocket_angle;
    }
  } else {
    if (x_position < mid_line) {
      if ((fmod(GetAngle() + 194.375, 360) < 180 && fmod(GetAngle() + 194.375, 360) >= 0) || fmod(GetAngle() + 194.375, 360) < -180) {
        frc::SmartDashboard::PutString("Zone", "Left Cargo Ship");
        return GetAngle() - left_cargo_ship_angle;
      } else {
        frc::SmartDashboard::PutString("Zone", "Back Left Rocket");
        return GetAngle() - back_left_rocket_angle;
      }
    } else {
      if ((fmod(GetAngle() + 165.625, 360) < 180 && fmod(GetAngle() + 165.625, 360) >= 0) || fmod(GetAngle() + 165.625, 360) < -180) {
        frc::SmartDashboard::PutString("Zone", "Back Right Rocket");
        return GetAngle() - back_right_rocket_angle;
      } else {
        frc::SmartDashboard::PutString("Zone", "Right Cargo Ship");
        return GetAngle() - right_cargo_ship_angle;
      }
    }
  }
}
