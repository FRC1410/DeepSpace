...
DriveTrain::DriveTrain() : Subsystem("DriveTrain") {
  m_chooser.SetDefaultOption("Gyro", "m_gyro");
  m_chooser.AddOption("Distance", "m_distance");
  m_chooser.AddOption("Limelight Distance", "m_limelight_distance");
  Shuffleboard::GetTab("Auto").Add("PID Mode", m_chooser).WithWidget(BuiltInWidgets::kComboBoxChooser).WithPosition(3, 2).WithSize(1, 1);
}
...
void DriveTrain::SetCurvedSpeed(double left, double right) {
	...
	SetSpeed(left_velocity, right_velocity);
}
...
double DriveTrain::ApplyAcceleration(double current_speed, double target_speed, double time_difference) {
  current_speed = Robot::m_macro_superstructure.CapValue(current_speed, 1);
  target_speed = Robot::m_macro_superstructure.CapValue(target_speed, 1);
...
double DriveTrain::GetLeftDistance() {
	return -(m_left_back.GetSensorCollection().GetQuadraturePosition() - initial_left_position) * (6 * pi / 1024);
}

double DriveTrain::GetRightDistance() {
  return (m_right_middle.GetSensorCollection().GetQuadraturePosition() - initial_right_position) * (6 * pi / 1024);
}

double DriveTrain::GetDistance() {
  shuffleboard_left_distance.SetDouble(GetLeftDistance());
  shuffleboard_right_distance.SetDouble(GetRightDistance());
  shuffleboard_average_distance.SetDouble((GetLeftDistance() + GetRightDistance()) / 2);
...
double DriveTrain::GetAngle() {
  shuffleboard_gyro.SetDouble(NavX->GetAngle() + initial_angle);
  return NavX->GetAngle() + initial_angle;
}

void DriveTrain::SetX(double value) {
  x_position = value;
  shuffleboard_x.SetDouble(x_position);
}

void DriveTrain::SetZ(double value) {
  z_position = value;
  shuffleboard_z.SetDouble(z_position);
}

void DriveTrain::IncrementX(double increment_value) {
  x_position += increment_value;
  shuffleboard_x.SetDouble(x_position);
}

void DriveTrain::IncrementZ(double increment_value) {
  z_position += increment_value;
  shuffleboard_z.SetDouble(z_position);
}
...
void DriveTrain::SetPIDConstants() {
  if (m_chooser.GetSelected() == previous_PID_mode) {
    if (previous_PID_mode == "m_gyro") {
      input_gyro_kP = shuffleboard_P_input.GetDouble(input_gyro_kP);
      input_gyro_kI = shuffleboard_I_input.GetDouble(input_gyro_kI);
      input_gyro_kD = shuffleboard_D_input.GetDouble(input_gyro_kD);
    } else if (previous_PID_mode == "m_distance") {
      input_distance_kP = shuffleboard_P_input.GetDouble(input_distance_kP);
      input_distance_kI = shuffleboard_I_input.GetDouble(input_distance_kI);
      input_distance_kD = shuffleboard_D_input.GetDouble(input_distance_kD);
    } else {
      input_limelight_kP = shuffleboard_P_input.GetDouble(input_limelight_kP);
      input_limelight_kI = shuffleboard_I_input.GetDouble(input_limelight_kI);
      input_limelight_kD = shuffleboard_D_input.GetDouble(input_limelight_kD);

      Robot::m_limelight.SetDistancePIDConstants(input_limelight_kP, input_limelight_kI, input_limelight_kD);
    }
  } else {
    if (m_chooser.GetSelected() == "m_gyro") {
      shuffleboard_P_input.SetDouble(input_gyro_kP);
      shuffleboard_I_input.SetDouble(input_gyro_kI);
      shuffleboard_D_input.SetDouble(input_gyro_kD);
    } else if (m_chooser.GetSelected() == "m_distance") {
      shuffleboard_P_input.SetDouble(input_distance_kP);
      shuffleboard_I_input.SetDouble(input_distance_kI);
      shuffleboard_D_input.SetDouble(input_distance_kD);
    } else {
      shuffleboard_P_input.SetDouble(input_limelight_kP);
      shuffleboard_I_input.SetDouble(input_limelight_kI);
      shuffleboard_D_input.SetDouble(input_limelight_kD);
    }
  }
  
  if (m_chooser.GetSelected() == "m_gyro") {
    shuffleboard_P.SetDouble(input_gyro_kP);
    shuffleboard_I.SetDouble(input_gyro_kI);
    shuffleboard_D.SetDouble(input_gyro_kD);
  } else if (m_chooser.GetSelected() == "m_distance") {
    shuffleboard_P.SetDouble(input_distance_kP);
    shuffleboard_I.SetDouble(input_distance_kI);
    shuffleboard_D.SetDouble(input_distance_kD);
  } else {
    shuffleboard_P.SetDouble(input_limelight_kP);
    shuffleboard_I.SetDouble(input_limelight_kI);
    shuffleboard_D.SetDouble(input_limelight_kD);
  }

  previous_PID_mode = m_chooser.GetSelected();
}
...
double DriveTrain::CalcSkew(double time, double aspect) {
  ...
  if (CalcPathX(skew, time) > CalcPathZ(skew, time) * aspect) {
    ...
    if (CalcPathX(skew, time) < CalcPathZ(skew, time) * aspect) {
      ...
      while (abs(calced_x - calced_z * aspect) > tolerance) {
...
bool DriveTrain::CalculateRawPath(double target_x, double target_z, double angle, double final_velocity, double final_rotational_speed) {
  ...
  if (CalcSkew(lower_time, target_x / target_z) > 0 && CalcPathX(CalcSkew(lower_time, target_x / target_z), lower_time) < target_x && CalcSkew(upper_time, target_x / target_z) > 0 && CalcPathX(CalcSkew(upper_time, target_x / target_z), upper_time) > target_x) {
    ...
    shuffleboard_time.SetDouble(stable_time);
    return true;
  } else {
    jumper:
      return false;
...
void DriveTrain::FollowPath(double time, double time_difference) {
  shuffleboard_current_time.SetDouble(time);
  if (time < stable_i1) {
...
void DriveTrain::DisplayData(double speed, double rotational_speed, bool inverted) {
  frc::SmartDashboard::PutBoolean("Inverted Driving", inverted);

  shuffleboard_speed.SetDouble(speed);
  shuffleboard_rotational_speed.SetDouble(rotational_speed);
  shuffleboard_inverted.SetBoolean(inverted);
  shuffleboard_inverted_driving.SetBoolean(inverted);
}